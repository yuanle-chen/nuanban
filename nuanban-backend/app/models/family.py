...
class FamilyRelation(Base):
    ...
    child_user_id = Column(BigInteger, ForeignKey("users.id"), nullable=False, comment="子女用户ID")
...
class ElderProfile(Base):
    ...
    user_id = Column(BigInteger, ForeignKey("users.id"), nullable=False, unique=True, comment="用户ID")
    real_name = Column(String(50), nullable=False, comment="真实姓名")
    age = Column(BigInteger, comment="年龄")
